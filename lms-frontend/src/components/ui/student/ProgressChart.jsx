...
const ProgressChart = ({ course }) => {
  const { courseTitle, totalChapters, completedChapters, completionPercentage, chapters } = course;
  ...
  const checkIsAccessible = (chapter, index) => {
    return chapter.isStarted || index === 0 || (index > 0 && chapters[index - 1].isCompleted);
  };
  ...
  const getChapterStatus = (chapter, index) => {
    ...
    if (checkIsAccessible(chapter, index)) {
      return {
        text: chapter.isStarted ? 'In Progress' : 'Not Started',
        color: 'text-gray-500',
      };
    }
    return {
  ...
  return (
    <Card className="p-6">
      ...
      <div className="mb-8">
        ...
        <ProgressBar progress={completionPercentage} size="lg" />
      </div>
              ...
              return (
                <div key={chapter.chapterId} className="relative pl-10">
                  ...
                  >
                    ...
                    {!chapter.isCompleted && isAccessible && (
                      <div className="mt-3">
                        <ProgressBar progress={chapter.isStarted ? 50 : 0} size="sm" />
                      </div>
                    )}
                  </div>
                </div>
